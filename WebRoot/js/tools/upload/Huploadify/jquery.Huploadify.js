...
$.fn.Huploadify = function(opts){
	...
	this.each(function(){
		...
		var inputStr = '<input id="select_btn_'+instanceNumber+'" class="selectbtn" style="display:none;" type="file" capture="camera"  name="fileselect[]"';
				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			layer.alert('文件超出规定, 最大为'+option.fileSizeLimit/1024+'MB！请重新上传！');
				  					//alert('文件'+thisFile.name+'大小超出限制, 最大为'+option.fileSizeLimit/1024+'MB！');
				  			this.fileInput.val("");
				  			continue;
				  		}
						if($.inArray(thisFile.name.split('.').pop(),typeArray)>=0){
							...
							layer.alert('文件'+thisFile.name+'类型不允许！');
					  ...
					  if (xhr.readyState == 4) {
						  if (xhr.status == 200) {
							  ...
							  thisfile.find('.uploadify-progress-bar').css('width','100%');
								...
								responseText = responseText.substring(responseText.indexOf("[")+1, responseText.indexOf("]"))
							  option.onUploadSuccess&&option.onUploadSuccess(file, responseText);
							  //在指定的间隔时间后删掉进度条
							  setTimeout(function(){
							  	_this.find('#fileupload_'+instanceNumber+'_'+file.index).fadeOut();
							  },option.removeTimeout);
						  } else {
							  option.onUploadError&&option.onUploadError(file, responseText);		
						  }
						  option.onUploadComplete&&option.onUploadComplete(file,responseText);
						  //清除文件选择框中的已有值
